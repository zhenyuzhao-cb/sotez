...
var protocolOpTagMap = (_a = {},
    ...
    _a[constants_1.protocols['008a']] = opTag005,
    _a[constants_1.protocols['008']] = opTag005,
    _a[constants_1.protocols['009']] = opTag009,
    _a[constants_1.protocols['009a']] = opTag009,
    _a[constants_1.protocols['017']] = opTag009,
    _a);
...
var protocolOriginationMap = (_b = {},
    _b[constants_1.protocols['001']] = origination001,
    _b[constants_1.protocols['002']] = origination001,
    _b[constants_1.protocols['003']] = origination001,
    _b[constants_1.protocols['004']] = origination001,
    _b[constants_1.protocols['005a']] = origination005,
    _b[constants_1.protocols['005']] = origination005,
    _b[constants_1.protocols['006']] = origination005,
    _b[constants_1.protocols['007a']] = origination005,
    _b[constants_1.protocols['007']] = origination005,
    _b[constants_1.protocols['008a']] = origination005,
    _b[constants_1.protocols['008']] = origination005,
    _b);
/**
 ...
 */
var toBytesInt32Hex = function (num) {
    var forgedBuffer = new Uint8Array(exports.toBytesInt32(num));
    return utility_1.buf2hex(forgedBuffer);
};
exports.toBytesInt32Hex = toBytesInt32Hex;
/**
 * @description Convert bytes from Int16
 * @param {number} num Number to convert to bytes
 * @returns {Object} The converted number
 */
var toBytesInt16 = function (num) {
    // @ts-ignore
    num = parseInt(num, 10);
    var arr = new Uint8Array([(num & 0xff00) >> 8, num & 0x00ff]);
    return arr.buffer;
};
 ...
 * @returns {string} The converted number
 */
var toBytesInt16Hex = function (num) {
    var forgedBuffer = new Uint8Array(exports.toBytesInt16(num));
    return utility_1.buf2hex(forgedBuffer);
};
exports.toBytesInt16Hex = toBytesInt16Hex;
/**
 * @description Forge boolean
 * @param {boolean} boolArg Boolean value to convert
 * @returns {string} The converted boolean
 */
var bool = function (boolArg) { return (boolArg ? 'ff' : '00'); };
exports.bool = bool;
/**
 ...
 */
var parameters = function (parameter, protocol) {
    var _a;
    ...
    var protocolMap = (_a = {},
        ...
        _a[constants_1.protocols['008a']] = parameters005,
        _a[constants_1.protocols['008']] = parameters005,
        _a[constants_1.protocols['009']] = parameters005,
        _a[constants_1.protocols['009a']] = parameters005,
        _a[constants_1.protocols['017']] = parameters005,
        _a);
 ...
 */
var publicKeyHash = function (pkh) {
 ...
 */
var address = function (addressArg) {
    var fa = [];
 ...
 */
var op = function (opArg, protocol) {
    if (!protocolOpTagMap[protocol]) {
        throw new Error("Unrecognized protocol: " + protocol);
    }
    var fop = [];
    var forgedBuffer = protocolOpTagMap[protocol](opArg.kind);
    ...
    if (opArg.kind === 'endorsement') {
        ...
        fop.push(exports.delegation(opArg));
    }
    return fop.join('');
};
exports.op = op;
 ...
 * @returns {string} Forged operation bytes
 */
var endorsement = function (opArg) {
 ...
 * @returns {string} Forged operation bytes
 */
var seedNonceRevelation = function (opArg) {
    var fop = [];
    var levelBuffer = new Uint8Array(exports.toBytesInt32(opArg.level));
 ...
 * @returns {string} Forged operation bytes
 */
var transaction = function (opArg, protocol) {
    var fop = [];
    fop.push(exports.publicKeyHash(opArg.source));
    fop.push(exports.zarith(opArg.fee));
    fop.push(exports.zarith(opArg.counter));
 ...
 * @returns {string} Forged operation bytes
 */
var origination = function (opArg, protocol) {
    var fop = [];
    fop.push(exports.publicKeyHash(opArg.source));
    fop.push(exports.zarith(opArg.fee));
    fop.push(exports.zarith(opArg.counter));
 ...
 */
var forge = function (opOb, counter, protocol) { return __awaiter(void 0, void 0, void 0, function () {
 ...
 */
var decodeRawBytes = function (bytes) {
    ...
    var rec = function () {
        ...
        if (prim instanceof Object) {
            ...
            var args = __spreadArray([], Array(prim.len));
            var result = {
                prim: forgeOp,
...
exports.decodeRawBytes = decodeRawBytes;
/**
                ...
                if (inputArg.prim === 'LAMBDA') {
                    ...
                    if (inputArg.args) {
                        ...
                        inputArg.args.forEach(function (arg) {
                            innerResult_1.push(rec(arg));
                        });
                        var len = innerResult_1.join('').length / 2;
                        result.push(len.toString(16).padStart(8, '0'));
                        innerResult_1.forEach(function (x) { return result.push(x); });
                    }
                    var annotsBytes = inputArg.annots
                        ? inputArg.annots
                            .map(function (x) { return utility_1.buf2hex(new Uint8Array(utility_1.textEncode(x))); })
                            .join('20')
                        : '';
...
exports.encodeRawBytes = encodeRawBytes;
exports.default = {
